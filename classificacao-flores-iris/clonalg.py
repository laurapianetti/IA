...
def clonalg(num_anticorpos, num_geracoes, m, total_clones, X_train, y_train):
    ...
    historico = []  # Armazena informações para fazer o gráfico
    ...
    for _ in range(num_geracoes):
        ...
        historico.append(melhores_anti.copy())  # Armazena os melhores anticorpos de cada geração
...
if __name__ == "__main__":
    ...
    melhor_solucao, historico = clonalg(num_anticorpos, num_geracoes, m, total_clones, X_train, y_train)
    ...
    fig.suptitle('Evolução dos Anticorpos - Algoritmo Imunológico')
    cores = ['red', 'green', 'blue']
    for i, cor in enumerate(cores):
        ax.scatter(X_train[y_train == i, 0], X_train[y_train == i, 1], c=cor, label=f'Classe {i}', alpha=0.3)
    scatters = [ax.scatter([], [], c=cor, edgecolor='k', s=120, marker='X') for cor in cores]
    ...
    ax.set_title('Evolução dos Protótipos (Anticorpos) - Algoritmo Imunológico')
    ax.legend()

    def update(frame):
        for i, scatter in enumerate(scatters):
            scatter.set_offsets(historico[frame][i][:, :2])  # Só as duas primeiras dimensões
        ax.set_title(f'Evolução dos Protótipos - Geração {frame+1}')
        return scatters

    anim = FuncAnimation(fig, update, frames=len(historico), interval=400, blit=False)

    # Para salvar como mp4 (requer ffmpeg instalado)
    anim.save('evolucao_clonalg_iris.mp4', writer='ffmpeg')
